/**
 * heroMap.js
 * Mapa interactivo de las 3 ciudades en la home.
 * Bidireccional: cards <-> mapa.
 */
...
function highlightCard(cityKey) {
  document.querySelectorAll('.city-card').forEach((card) => {
    card.classList.toggle('city-card--active', card.dataset.city === cityKey);
  });
}
...
function initHeroMap() {
  ...
  L.polyline([[49.27, -123.12], [33.96, -118.35]], { color: '#FF2A00', weight: 1, dashArray: '6,6', opacity: 0.4 }).addTo(heroMapInstance);
  L.polyline([[33.96, -118.35], [31.69, -106.42]], { color: '#FFD600', weight: 1, dashArray: '6,6', opacity: 0.4 }).addTo(heroMapInstance);
  L.polyline([[49.27, -123.12], [31.69, -106.42]], { color: '#00E5FF', weight: 1, dashArray: '6,6', opacity: 0.3 }).addTo(heroMapInstance);

  // Card hover -> map flyTo
  document.querySelectorAll('.city-card').forEach((card) => {
...
export function initHeroMapModule() {
