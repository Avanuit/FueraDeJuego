...
for (const _cat in import_helpers.tokenCategories) for (const _k in import_helpers.tokenCategories[_cat]) tokenCategoryByKey[_k] = _cat;
...
const getTokenForKey = (key, value, styleProps, styleState) => {
  ...
  if (themeValue) {
    ...
    if (value in conf.specificTokens) {
      ...
      switch (key) {
        case "fontFamily":
          valOrVar = (context?.language ? (0, import_getVariantExtras.getFontsForLanguage)(conf.fontsParsed, context.language) : conf.fontsParsed)[value]?.family || value;
  ...
  if (hasSet) {
    ...
    if (opacityModifier !== void 0 && opacityModifier < 1) out = (0, import_normalizeColor.normalizeColor)(String(out), opacityModifier) ?? out;
    if (process.env.NODE_ENV === "development" && styleState.debug === "verbose") {
      globalThis.tamaguiAvoidTracking = true;
